...
class MediaFileAdapter(val logger: Logger) : PagingDataAdapter<MediaFile, MediaFileViewHolder>(DIFF_CALLBACK) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MediaFileViewHolder =
        MediaFileViewHolder(
            MediaFileBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
